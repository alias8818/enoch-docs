...
| --- | --- | --- | --- |
| `enoch-core` (reference control VM, host alias in source repo: `enoch-core.exe.xyz`) | Control plane | `/opt/enoch-control-plane` | Runs the FastAPI control plane, dashboard/API, local Postgres runtime storage, automation timers, and paper/corpus tooling. The `.exe.xyz` form is a private deploy alias; do not publish it elsewhere. |
