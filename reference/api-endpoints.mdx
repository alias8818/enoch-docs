...
Unauthenticated routes include `GET /healthz`, `GET /dashboard`, `GET /control/dashboard`, `GET /control/dashboard-v2`, and `GET /favicon.ico`. Dashboard data APIs and most control-plane routes require a bearer token or a scoped dashboard session path where the dashboard code supports it.
...
The dashboard keeps the operator bearer token in memory and scrubs legacy browser storage/cookies when the token is entered. A backend-mediated HttpOnly session cookie can preserve dashboard refresh continuity without exposing the bearer token to JavaScript. Do not pass bearer tokens in dashboard URLs or persist them in localStorage/sessionStorage.
...
Current deployments may set `store_backend` to `supabase` while using Postgres-backed storage; treat that value as a compatibility adapter label, not a Supabase Cloud claim.
...
<Accordion title="Bounded operator read models">
...
| Field | Meaning |
|---|---|
| `operator_counts` | Canonical operator lanes such as attention, running, write, publish, and imported |
| `operator_detail_counts` | Drill-down compatibility/detail counts |
| `paper_pipeline.write_needed` | Actionable positive paper work only |
| `paper_pipeline.raw_completed_no_paper_candidates` | Informational/debug rows before the decision gate |
| `paper_pipeline.not_writable_by_decision_gate` | Rows rejected by the decision gate |
| `paper_pipeline.publish_ready` | Finalized drafts missing corpus import |
| `paper_pipeline.published_imported` | Papers already represented in the corpus-import ledger |
...
### `GET /control/dashboard-v2`

Returns the canonical browser-based Enoch control dashboard shell as an HTML page. No authentication is required to load the page; the dashboard prompts for a bearer token and keeps it in memory for API calls. When the backend session endpoints are enabled, the dashboard can use an HttpOnly session cookie to recover after refresh without storing the bearer token in JavaScript-readable storage.

```bash
open http://127.0.0.1:8787/control/dashboard-v2
```
...
Legacy dashboard URL. Current deployments redirect to `/control/dashboard-v2`.

---

### `GET /dashboard`

Legacy worker-gate dashboard URL. Current deployments redirect to `/control/dashboard-v2`.

---

### `GET /dashboard/api`

Returns the full legacy worker dashboard snapshot as JSON. Use bounded `/control/api/v1/*` endpoints for primary operator dashboards and integrations.
