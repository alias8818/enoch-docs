...
This page maps the main endpoint groups present in the local source snapshot. It is not a generated OpenAPI reference; verify request and response models against the code before building clients. The operator dashboard prefers bounded `/control/api/v1/*` read models, while older broad status surfaces remain available for compatibility and debug. Major groups not fully documented here include `POST /control/dispatch-one`, `POST /control/api/alerts/queue-check`, Research Facility routes under `/control/api/research/*`, and legacy dashboard routes such as `/dashboard/api`.
...
- Legacy event model alias `OmxEvent` (`ControlPlaneEvent`) remains for historical payloads; the HTTP route `POST /omx/event` was removed. Worker completion callbacks use `POST /control/api/worker-callback`.
...
Response `action` values: `"paused"`, `"noop"`, `"dry_run_dispatch"`, `"live_dispatch"`. Live dispatch failures return HTTP `409`/`501`/`502` rather than an `action` of `dispatch_blocked`.
...
| `page_size` | `50` | Rows per page (1–500) |
| `search` | `""` | Full-text filter |
| `review_status` | `""` | Automation state filter. Canonical values: `queued`, `claimed`, `blocked`, `finalized`, `deferred`, `rejected`. Legacy imported rows may still use values such as `triage_ready`, `unreviewed`, `in_review`, `changes_requested`, and `approved_for_finalization`. |
...
### Legacy `POST /omx/event` (removed)

The `ControlPlaneEvent` model remains in code (`OmxEvent` is a deprecated alias), but current releases do **not** register an HTTP route at `/omx/event`. Worker completion callbacks should target `POST /control/api/worker-callback` on the control plane with a bearer token that matches `control_api_bearer_token`.
