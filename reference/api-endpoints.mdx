...
Optional request field `machine_target` selects a named entry from `worker_targets` instead of `wake_gate_url`.
...
Response `action` values: `"noop"`, `"dry_run_draft"`, `"drafted"`. When `paper_evidence_sync_enabled` is true and local evidence is missing, live calls can return HTTP 424 with an `evidence_sync` detail object.
...
Paper automation routes are also available under `/control/api/publication-automation/*` as aliases for `/control/api/paper-reviews/*`.
