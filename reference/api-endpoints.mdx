...
Returns the latest intake observation for the legacy Notion sync path, the current queued-project projection, skipped-row reason counts, and recent intake events. Supabase-native intake should normally be verified through the Supabase idea workbench and queue tables instead of this compatibility endpoint.
