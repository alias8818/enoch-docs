...
Use the configured `control_api_bearer_token` as a bearer token for dashboard and control API calls. When worker callbacks target `/control/api/worker-callback`, the worker must send the same token as `completion_callback_token`. Keep `worker_wake_gate_bearer_token` separate for control-plane → worker API calls. The HTML shell is served at `/dashboard`; dashboard data routes require token-bearing requests.
