...
For local development, set `worker_wake_gate_url` and `completion_callback_url` to the local service and point callbacks at `/control/api/worker-callback`. Set `completion_callback_token` equal to `control_api_bearer_token` on the receiving control plane. For production with multiple workers, configure `worker_targets` and route dispatches through `workload_machine_targets` or explicit `machine_target` in preflight requests.
