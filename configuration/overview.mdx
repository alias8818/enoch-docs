...
Run this command separately for each trust boundary. The control-plane inbound token and completion callback token should be distinct. In a two-machine deployment, `worker_wake_gate_bearer_token` on the control VM must match `omx_inbound_bearer_token` on the worker.

## Required fields

These fields have no usable defaults and must be set before the service will start.
...
These fields have sensible defaults for local development but you should set them explicitly in production.

<ParamField path="state_dir" type="string" default="~/.local/state/omx-wake-gate">
  Absolute or `~`-prefixed path to the local durable state directory. The service stores queue state, run records, and internal databases here.
</ParamField>

<ParamField path="project_root" type="string" default="~/enoch/projects">
  Root directory for project workspaces and generated paper artifacts. Each project gets a subdirectory under this path.
</ParamField>

<ParamField path="dispatch_script_path" type="string" default="~/enoch/bin/enoch_omx_dispatch.sh">
  Path to the shell script the control plane executes to launch agent runs. Ensure the script is executable and accessible by the service user.
</ParamField>
