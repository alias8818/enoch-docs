...
The corpus packaging/provenance policy requires generated artifacts to avoid fake citations, placeholder markers, implied human authorship, and peer-review claims. It also expects provenance metadata and evidence/claim-ledger availability or an explicit statement that those artifacts are missing.

## Current local corpus snapshot

<Warning>
Do not publish or finalize a paper until packaging/provenance checks and human review pass. A paper with placeholder citations or missing provenance artifacts misrepresents the evidence behind its claims. The control plane tracks review status and supports finalization-package preparation, but the release snapshot still listed corpus import, index building, packaging/provenance scans, and the full GLM-5.1 paper rewrite loop as pending before public release.
</Warning>
