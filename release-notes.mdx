...
- Legacy `.omx/project_decision.json`, `omx_inbound_bearer_token`, and the `OmxEvent` model alias remain for older runs; the HTTP route `POST /omx/event` was removed — use `POST /control/api/worker-callback` instead.
