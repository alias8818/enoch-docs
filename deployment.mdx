...
## What the repository does not include
  ...
  <Step title="Configure the paper artifact workflow">
    The control plane can rewrite and package generated research artifacts when paper rows and evidence are present. This path depends on provider credentials and, in the release snapshot, the full GLM-5.1 rewrite loop was still listed as pending before public release. Add the following settings to `/etc/enoch/config.json` only when you are ready to test the provider-backed writer:
    ...
    <Warning>
      Do not publish generated artifacts until corpus import, index building, quality scans, and human review are complete. The quality gates scan for placeholder citations, missing provenance, and missing evidence artifacts; the release snapshot still listed several corpus-publication steps as pending.
    </Warning>
  </Step>
</Steps>
